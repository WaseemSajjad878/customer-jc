...
import { addAircraft, getAircraftById, getAircraftFiles, getAircrafts, updaeAircraft } from '@/utils/aircraft'
...
export const useAircraftStore = defineStore({
  ...
  getters: {
    getAircrafts: (state) =>
      state.search ? state.aircrafts.filter((x) => x.title.includes(state.search)) : state.aircrafts,
